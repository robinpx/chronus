...
void loop() {
  ...
  while (Serial1.available() > 0 && !gpsCaptured){
    ...
    if (totalGPGSVMessages.isUpdated()) {
       ...
       if (clearSNR != 0 && satNum != 0) {
          ...
          Serial.println(F("Valid data captured:"));
          Serial.print(F("satNum="));
          Serial.print(satNum);
          Serial.print(F(" clearSNR="));
          Serial.print(clearSNR);
          Serial.println();
       }
    }
  }
  ...
  if (gpsCaptured && (cleaning || docking || charging)) { //! REMOVE CHARGING AFTER DEBUG
